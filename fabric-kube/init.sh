...
externalPeerOrgs=$(yq r -c ./crypto-config.yaml 'ExternalPeerOrgs // empty')
...
externalOrdererOrgs=$(yq r -c ./crypto-config.yaml 'ExternalOrdererOrgs // empty')
...
if [ "$create_genesis_block" == true ]; then
    ...
    genesisProfile=$(yq r $config_file 'network.genesisProfile')
    systemChannelID=$(yq r $config_file 'network.systemChannelID')
